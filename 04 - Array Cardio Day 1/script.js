...
const orderedInvestors = inventors.sort((a, b) => {
    return a.year - b.year;
})

// Array.prototype.reduce()
// 4. How many years did all the inventors live all together?
const yearsAliveSum = inventors.reduce((acc, curr) => {
    const yearsAlive = curr.passed - curr.year;
    return acc + yearsAlive;
}, 0)
console.log(yearsAliveSum);
...
// practicing
const inventorsByName = inventors.sort((a, b) => {
...
const numOfInstances = data.reduce((acc, curr) => {
    if (!(curr in acc)) {
        acc[curr] = 1;
    } else {
        acc[curr]++;
